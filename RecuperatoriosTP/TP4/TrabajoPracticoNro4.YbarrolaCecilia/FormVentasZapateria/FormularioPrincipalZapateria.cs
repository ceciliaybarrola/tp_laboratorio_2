...
namespace FormVentasZapateria
{
    ...
    public partial class FormularioPrincipalZapateria : Form
    {
        ...
        /// </summary>
        public void ActualizarDataGridDB()
        {       
        ...
        /// </summary>
        public void ActualizarListaZapateriaDB()
        {
            this.zapateria.stock = AccesoDatos.ObtenerListaCalzados();
        }
        /// <summary>
        ...
        /// </summary>
        public void ActualizarBalanceDelDia()
        {
            if (this.labelGanancias.InvokeRequired)
            {
                delegadoActualizaciones delegado = new delegadoActualizaciones(this.ActualizarBalanceDelDia);
        ...
        /// </summary>
        public void ActualizarVentasDelDia()
        {
            if (this.labelGanancias.InvokeRequired)
            {
                delegadoActualizaciones delegado = new delegadoActualizaciones(this.ActualizarVentasDelDia);
                this.Invoke(delegado);
            }
            else
            {
            ...
            try
            {
                if(this.listaCarritoCompras.Count > 0)
                {
                    ...
                    foreach(Calzado item in this.listaCarritoCompras)
                    {
                        ...
                        item.Cantidad = (AccesoDatos.ObtenerCalzadoPorID(item.Id)).Cantidad - cantidad;
                        AccesoDatos.ModificarCalzado(item);
                        item.Cantidad = cantidad;
                        //Realizo esta asignacion para cambiar al valor de item.Cantidad ya modificado al valor original de ese elemento
                        //y asi tener en la lista de ventas la cantidad vendid originalmente, sin la necesidad de crear un nuevo objeto 
                        //para ingresarlo en la base de datos solo por modificar la cantidad
                    }
